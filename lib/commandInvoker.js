...
function extractTextFromMessage(msg) {
  ...
  const message = normalizeMessageContent(msg.message) || msg.message || {};
...
async function invokeCommand(commandHandler, sock, from, msg, isAdmin, q, session, args, botData, saveBotData, userId) {
  ...
  const usesCanonicalGroupSignature = [
    ...
    'groupinfo', 'groupinvite', 'grouprevokeinvite', 'groupacceptinvite',
    'groupleave', 'groupcreate', 'groupadd', 'groupremove', 'grouppromote',
    'groupdemote', 'groupapprove', 'groupsettings', 'groupmemberaddmode',
    'groupephemeral', 'groupjoinapproval', 'grouprequests', 'groupinviteinfo',
    'groupfetchall', 'groupremovephoto', 'groupupdatememberlabel', 'rendercommand',
    'airichpreviewcommand', 'uploadstatuscommand', 'tostatuscommand', 'objectcommandadapter'
  ].includes(handlerName);
  const usesChatSenderArgsSignature = handlerName === 'statscommand';

  if (usesChatSenderArgsSignature) {
    attemptSignatures.push(() => commandHandler(sock, chatId, senderId, safeArgs, safeMsg));
  } else if (usesCanonicalGroupSignature || (paramLength >= 4 && handlerName === 'command')) {
