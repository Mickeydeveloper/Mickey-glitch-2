...
const getButtonId = (m) => {
    ...
    if (message.singleSelectReply?.selectedRowId) {
        return message.singleSelectReply.selectedRowId;
    }
...
async function executeButtonHandler(buttonId, sock, chatId, m, handlers) {
    ...
    try {
        ...
        const handlerKey = Object.keys(handlers || {}).find((key) => key.toLowerCase() === String(buttonId).toLowerCase());
        if (handlerKey) {
            console.log(`✅ [Button] Executing handler for: ${buttonId}`);
            await handlers[handlerKey](sock, chatId, m);
            return true;
        }
