...
class MongoStore {
    ...
    async connect() {
        ...
        await this.db.collection('accounts').createIndex({ 'account.token': 1 });
    ...
    async findAccountByToken(token) {
        ...
        const row = await this.db.collection('accounts').findOne({
            $or: [
                { 'account.token': token },
                { 'account.accountToken': token },
                { 'account.accessToken': token },
                { 'account.token': { $regex: `^${escapedToken}$`, $options: 'i' } },
                { 'account.accountToken': { $regex: `^${escapedToken}$`, $options: 'i' } },
                { 'account.accessToken': { $regex: `^${escapedToken}$`, $options: 'i' } }
            ]
        });
        if (!row?.account) return null;
        return { accountId: row.accountId, account: row.account };
    }
